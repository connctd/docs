...
public: If set to true the connector is visible in connector store. If set to false its only visible in connector store if the connector belongs to you. Even if a connector is not shown in the store it can still be installed if its uuid and configuration is shared.
